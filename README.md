...
- [결과물 8종](examples/README.md) — 제공받은 기존 시연 자료 6종과 이 저장소의 스킬로 만든 예시 2종을 살펴봅니다.
...
- 스킬 소개는 `assets/js/catalog.js`에서 수정합니다. `status`(shared·example·planned), 소개 문단, 필요한 입력, 폴더 구성, 작업 흐름, 사용 예시, 연결할 결과물 `results`를 항목별로 적습니다. 스킬 상세 페이지 `skill.html?id=<id>`가 이 데이터로 그려집니다.
- 결과물은 `assets/js/results.js`에서 관리합니다. `origin`으로 제공된 시연 자료(provided)와 스킬로 만든 예시(generated)를 구분하고, `skill`로 관련 스킬을 연결합니다.
